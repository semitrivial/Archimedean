...
$i$th time. If $i=2^j$ for some integer $j$, then the agent shall receive an
infinite amount of reward, but otherwise, the agent shall receive $0$ reward.
If we approximate the infinite reward with a real-valued reward of, say, $1,000,000$,
...
something which is only possible in Example \ref{funnyexample} because the example
is so simple that we can completely understand the environment ourselves. For the
...
real numbers. Perhaps some recent work \cite{bookmark} on non-Archimedean
probability could be used to remove that roadblock.}.
