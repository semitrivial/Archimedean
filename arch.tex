...
\begin{proof}
    ...
    $i$ such that $f(x_\infty)\leq f(x_i)-r$, i.e., there is no
    $i$ such that $f(x_i)\geq f(x_\infty)+r$.
...
each of these theories. In the sense of Example \ref{nonexamples}, each $T_{i+1}$ is
significantly stronger than $T_i$, and $T_\infty$ is significantly stronger than
$T_0,T_1,\ldots$.}
...
Since $a\geq 0$, it follows that all the $x_i,x_{i+1}\geq 0$, and since
...
Brendon Miller-Boldt, and Stewart Shapiro for comments and feedback.
