...
agent is allowed to make might include one action for each key on a piano, and
an additional ``stand and bow'' action for signalling that a song is finished.
Whenever the agent stands and bows, the agent is awarded with applause based on
the beauty of the song the agent composed. Assuming musical beauty is
