...
\jagiheading{11}{1}{70--83}{2020}{2020-02-16}{2020-09-29}{10.2478/jagi-2020-0004}
{S.\ Alexander}
...
\[
\sum_{j=-m}^\infty a_j \epsilon^j.
\]
Suppose $A=\sum_{j=-m}^\infty a_j\epsilon^j$
and $B=\sum_{j=-n}^\infty b_j\epsilon^j$ are two distinct formal Laurent series.
We declare $A<B$ if and only if $a_j<b_j$ where $j$ is the smallest
index such that $a_j\not=b_j$ (where we consider $a_j$ to be $0$ for all
$j<-m$ and we consider $b_j$ to be $0$ for all $j<-n$).
\end{definition}

For brevity, we will write $a\epsilon^b$ for the formal Laurent series
$\sum_{j=-m}^{\infty} a_j\epsilon^j$ where $a_b=a$ and $a_j=0$ for all $j\not=0$.
Likewise, we may write, for example, $5\epsilon^{-1}+2\epsilon^3$ for the
formal Laurent series $\sum_{j=-m}^{\infty} a_j\epsilon^j$ where $a_{-1}=5$,
...
\begin{enumerate}
\item
For every Laurent series $A=\sum_{j=-m}^\infty a_j\epsilon^j$, let $o(A)=-m$ (call this the
\emph{order} of $A$), and let $LC(A)$ be the $\epsilon^{o(A)}$-coefficient of $A$ (call
this the \emph{leading coefficient} of $A$).
\item
Let $r>0$ be any positive real number.
For any formal Laurent series $A=\sum_{j=-m}^\infty a_j\epsilon^j$
and $B=\sum_{j=-n}^\infty b_j\epsilon^j$,
...
inadequacy of real number rewards, but of rewards from any $2$-Archimedean number
...
for the strength of mathematical theories can be shown to be non-$2$-Archimedean.

In the following theorem, we show that the formal Laurent series are $2$-Archimedean.
By the above paragraph, we consider this to be evidence that not only is
...
It can be shown that the hyperreals are not even $2$-Archimedean (much less Archimedean).
